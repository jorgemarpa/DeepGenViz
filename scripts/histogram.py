...
def histogram_tab(meta):
    ...
    def make_dataset(carrier_list, range_start=-60, range_end=120, bin_width=5):
        ...
        for i, carrier_name in enumerate(carrier_list):
            ...
            # Format the interval
            arr_df['f_interval'] = ['%d to %d minutes' %
                                    (left, right) for left, right in zip(arr_df['left'], arr_df['right'])]
    ...
    def update(attr, old, new):
        types_to_plot = [type_selection.labels[i] for i in type_selection.active]
    ...
    initial_carriers = [type_selection.labels[i] for i in type_selection.active]
